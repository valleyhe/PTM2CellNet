...
**Last updated: 2026-09-16（U2–U7 修复批次：DEG estimand 冻结为 donor pseudobulk counts 并重算（GSE174367 全队列任何无偏口径下仍无 FDR≤0.05 行，observed gate 阻塞确认为队列统计功效问题而非代码缺陷）；新增 `src/data/scvi_context.py` + `scripts/prepare_scvi_context.py` 统一 scVI context 准备入口（缺基因 policy 与 `davf_batch` 绑定显式必填、manifest 记录 rationale），并生成 KO route 正式 context `outputs/ptm_activity/20260916_d1/scvi_context/gse174367_ko_context.h5ad`（61,472 cells × 4018、33 缺失轴基因显式零填）；DAVF 轴覆盖审计确认 KO={APOE}、KD=∅，APP/PSEN1/BACE1/MAPT 在本地 30 个 scPerturb 数据集从未被扰动（Workflow B 重训无训练输入）；Gate-E vocabulary-migration benchmark 从真实 CPLM+dbptm 注释组装 60,030 行（25 种 PTM 类型）并通过 evaluator 端到端验证；D3 探针实测发现 PerturbGen `train_mask` 在 Tesla P40 上被 `torch.compile`（triton 需 CC≥7.0，P40 为 6.1）硬阻塞，`TORCHDYNAMO_DISABLE=1` eager 模式为 P40 唯一路径）**

当前权威分析是仓库根目录的
[`project_analysis_20260916.md`](../project_analysis_20260916.md)（上一份核实证据
[`project_analysis_20260915.md`](../project_analysis_20260915.md)；20260916 报告
§2.3 已校正 20260915 报告"FDR 恒等于 1.0"的文字表述——真实最小 FDR 为
0.0855 但显著行仍为 0，结论不变）。
...
[`project_repair_report_20260913.md`](../project_repair_report_20260913.md)（已归档）；
综合对抗分析见 [`project_analysis_20260914.md`](../project_analysis_20260914.md)（历史），
当前状态以 [`project_analysis_20260916.md`](../project_analysis_20260916.md) 为准。
...
- [项目代码与文档综合分析（2026-09-16，当前权威）](../project_analysis_20260916.md)、[修复执行报告（2026-09-16，U2–U7 批次）](../project_repair_report_20260916.md)、[上一份核实证据（2026-09-15）](../project_analysis_20260915.md)、[归档清单（2026-09-16）](../archive/20260916/ARCHIVE_MANIFEST.md)、[归档清单（2026-09-14）](../archive/20260914/ARCHIVE_MANIFEST.md)、[归档清单（2026-09-13）](../archive/20260913/ARCHIVE_MANIFEST.md)
