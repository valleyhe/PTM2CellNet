...
**Last updated: 2026-08-21（归档轮 20260821 与综合分析刷新后）**

当前权威分析是仓库根目录的
[`project_analysis_20260821.md`](./project_analysis_20260821.md)。
2026-08-20 的综合分析已作为历史快照归档到
[`archive/20260821/`](./archive/20260821/MANIFEST.md)（原路径留有重定向入口）；
2026-08-16 的综合分析 v5.0 归档到
...
- **静态/构建质量**：`compileall` 通过；`ruff check src scripts tests` 通过（2026-08-21 复测全绿）；`mypy src` 实测结果依赖 mypy 版本——**mypy 1.20 为 25 errors / 12 files，mypy 2.1.0 为 5 errors / 2 files**（差异源于新版 mypy 对 `Tensor | Module` 联合与 `str | None` 收窄的推断改进；CI 安装最新 mypy，技术债 TD-N-06 以 25 errors 口径跟踪，详见 project_analysis_20260821.md）；`pip check` 仍有两个可选依赖冲突（已登记 F-10）。2026-08-21 首次 `pip-audit` 全量审计发现锁定清单 **64 个已知漏洞条目 / 12 个包**（TD-N-08）。
...
- [项目代码与文档综合分析（2026-08-21，当前权威）](../project_analysis_20260821.md)、[归档清单（2026-08-21）](../archive/20260821/MANIFEST.md)
