...
**Last updated: 2026-09-01（方向门控实现、归档批次 20260901、全量离线回归完成）**
...
- **测试基线（2026-09-01 更新）**：全量 `pytest tests`（`unshare -rn` 网络隔离 + HF 离线）**2357 passed / 16 skipped / 47 warnings / exit 0（536.65s）**，2373 项收集，零失败。
...
- **PerturbGen 双路径集成**：已增加 `PTM proposal → DAVF expression delta → observed direction` 三方 gate、strict `gene_to_token` mapper 和 `mainline` 决策函数；gate 通过后才允许构造 `CandidateEvidence`。**生产 `run_perturbgen_pipeline.py` 尚未自动调用 mainline**，M0⑥ ≥3 donor 合规 cohort 仍阻断（本地 30 文件审计 0 合规候选），真实 DAVF/Gate-E/Gate-4 仍未验收，详见 `project_analysis_20260901.md`。
- **R-01～R-03 状态**：工程契约与 synthetic batch 测试通过；三路 pLM 权重已本地化；真实图/扰动训练和科学验收仍未验证（依赖 replogle/scgenescope 数据供给）。
- **剩余需求差距**：正式 REQUIREMENTS v2.1/v2.2 共 24 项 Complete 维持成立；另有 2026-08-21 PerturbGen 后续方案未完成 donor、真实 DAVF、生产主线编排和 formal evidence，逐项清单见权威分析 §4。
