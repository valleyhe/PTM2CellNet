...
2026-09-04 重新构造了与 E2E 推理一致的 cell-baseline pair，并将候选模型写入
`*_cell_baseline_formal_20260904`。`configs/davf_ko.yaml` 和
...
六阶段，并在通过 DAVF gate 后按候选的独立输出目录执行。
...
最后才允许接入 PerturbGen：DAVF 只给目标变化方向和证据，PerturbGen 负责后续
全基因表达效用。当前默认配置仍指向历史 checkpoint，因此候选验收阶段应显式
传入新 checkpoint；待验收完成后再单独切换 `configs/davf_ko.yaml` 和
`configs/davf_kd.yaml`，推理时不能把两条路径的方向标签混用。

## 当前边界

- 新 checkpoint 已是真实数据上的可运行训练产物，并通过 endpoint/方向/bridge
  工程验收；仍不等于已通过生物学发布门。还需要独立 held-out target、至少三个
  seed，以及 scVI decode 后的目标基因 sign accuracy、相关性和接近零变化的
  inconclusive 比例。
