...
以下表格完整保留前一阶段主会话的 9 次记录（8 个直接子代理调用 + 1 次报告执行），包括原有成功次数和可计算时长；这些记录不属于本轮 continuation，也不与本轮 4 次 direct spawn 重复。本表把容量错误的 Nietzsche 调用计入“调用次数”，但不计入成功执行次数；报告生成由前一阶段主会话直接执行，按原记录计为 1 次。`平均执行时长`只在有可靠开始/结束事件时填写；前一阶段仅 Avicenna 和 Fermat 有可靠事件，其余不可计算，绝不估计。
...
| 前一阶段主会话报告执行者 | 1 | 1 | 读取依据、写入并核验本报告 | 不可计算（无可靠工具端到端时长） |
| **合计** | **9** | **8** | 8 个直接子代理调用 + 1 次报告执行 | 仅 2 个执行者有可计算时长 |

### 10.3 本轮 continuation direct spawn ledger

本轮 continuation 实际直接调用 `spawn_agent` 4 次：Epicurus、Meitner、Descartes、Lovelace 各 1 次；另有 Epicurus `resume` 1 次生命周期操作。`resume` 不作为新 spawn，也不计入 direct spawn 小计。成功完成只按完成通知或实际落地计数；Meitner 按代码/报告提交链实际落地计 1 次，但不声称返回了正常终态。

| 直接 spawn 执行者 | 调用次数 | 成功完成次数 | 主要任务与真实状态 | 平均执行时长 |
|---|---:|---:|---|---|
| Epicurus | 1 | 0 | 产生全量 2819 passed 证据后被中止；报告收口未完成 | 不可计算（工具仅返回 timeout/完成通知，无可靠端到端起止时间） |
| Meitner | 1 | 1 | 完成代码/报告提交链并实际落地；未返回正常终态 | 不可计算（工具仅返回 timeout/完成通知，无可靠端到端起止时间） |
...
| **direct spawn 小计** | **4** | **3** | 4 次直接 spawn；Epicurus 未完成报告收口 | 不可计算（工具仅返回 timeout/完成通知，无可靠端到端起止时间） |

**生命周期操作：** Epicurus `resume` 1 次；不计为新 spawn，也不计入上述 direct spawn/成功完成小计。

**本轮小计与去重：** direct spawn=4，resume=1；按“调用次数”只计 spawn=4，成功完成=3/4（Epicurus 未完成；Meitner 以实际落地计入，但没有正常完成通知）。前一阶段的 9 次是独立的历史主会话 ledger，本轮的 4 次是 continuation 的新 direct spawn，二者统计范围和调用集合不同，不重复计算。

### 10.4 统计解释

前一阶段的成功调用主要完成了当前工程交付所需的审计、归档和版本控制动作；Nietzsche 的 capacity error 没有产生审计结论，Darwin 承担了替代审计。时长不是由所有调用统一返回，故不以两项可靠时长推算其他 agent，也不把历史 ZMemory 的 started_at 当作本轮调用耗时。本轮 continuation 只报告 4 次实际 direct spawn 和 1 次 Epicurus resume；resume 是生命周期操作，不增加 spawn 调用次数。
