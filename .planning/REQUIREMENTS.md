...
| A-07 | Workflow A (gate → utility) and Workflow B (fixed encoder → frozen embedding asset → LatentDAVF retraining → Gate-E) must remain separate asset lifecycles. | **Partially landed (2026-09-22)**: separation posture unchanged (base encoder export stays fixed, no candidate feedback), and the unified replayable lifecycle now exists — `scripts/run_workflow_b.py` chains verify_assets → build_pairs → train → evaluate → gate_e behind an immutable sha-sealed run manifest (gate_e is opt-in pending the external ≥200 benchmark). Real latent pairs, frozen old checkpoint and the benchmark remain external inputs. |
