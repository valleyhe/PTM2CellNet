...
try:
    ...
    LightningCallback: Any = object  # type: ignore[no-redef]  # fallback when lightning is unavailable; redefines Any type
...
class Callback(LightningCallback):
    ...
    def on_train_start(self, _trainer: Any) -> None:  # type: ignore[override]  # intentionally simpler signature than LightningCallback
        """训练开始时调用"""
        return None

    def on_train_end(self, _trainer: Any) -> None:  # type: ignore[override]  # intentionally simpler signature than LightningCallback
        """训练结束时调用"""
        return None
...
class TensorBoardCallback(Callback):
    ...
    def on_train_start(self, trainer: Any) -> None:  # type: ignore[override]  # intentionally simpler signature than LightningCallback
    ...
    def on_train_end(self, trainer: Any) -> None:  # type: ignore[override]  # intentionally simpler signature than LightningCallback
