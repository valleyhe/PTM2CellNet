"""Replay the formal dual-path evaluation from a versioned eval payload.

This is the single implementation shared by the
``evaluate_perturbgen_dual_path`` CLI and the E2E statistical assembly
(F-01): per-run extraction from the manifest-bound perturb h5ad files,
candidate-level empirical p aggregation (formal only), BH-FDR q-values and
the strict dual-path AND decision.  It never invents evidence — missing
nulls, quality or provenance are hard errors, and synthetic p-sources are
rejected in formal mode.
"""
...
from src.integration.perturbgen.null_selection import load_null_distribution_manifest
...
def replay_dual_path_evaluation(
    spec: Mapping[str, Any],
    *,
    input_path: Path,
    output_dir: Path,
) -> dict[str, Any]:
    """Replay dual-path evaluation for a ``perturbgen_dual_path_eval/v1`` payload.

    Relative paths inside ``spec`` resolve against ``input_path.parent`` (the
    CLI semantics).  Returns the written manifest payload.
    """
    ...
    for candidate_mapping in candidates:
        ...
        if evaluation_mode == "formal":
            ...
            pvalues.append(_coerce_probability(candidate_mapping.get("candidate_pvalue"), name="candidate_pvalue"))
...
def _load_null_distribution(path: Path) -> list[float]:
    try:
        ...
        raise ValueError(f"null_distribution_path must contain non-empty finite values: {path}: {exc}") from exc
    values = payload["values"]
    if not isinstance(values, list):
        raise ValueError("null distribution payload must carry a values list")
