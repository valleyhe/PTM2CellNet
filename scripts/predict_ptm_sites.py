                        ...
                        results.append(
                            {
                                ...
                                "sequence_window": sequence[
                                    max(0, c["position"] - (DEFAULT_PTM_HALF_WINDOW + 1)) : c["position"]
        ...
        for protein_id, sequence in tqdm(zip(protein_ids, sequences, strict=False), total=len(sequences)):
            ...
            if len(df) > 0:
                ...
                for ptm_type in df["ptm_type"].unique():
                    stats["ptm_counts"][ptm_type] = stats["ptm_counts"].get(ptm_type, 0) + len(
                        df[df["ptm_type"] == ptm_type]
                    )
    ...
    def predict_variants(
        ...
        for row in tqdm(variants_df.to_dict("records"), total=len(variants_df)):
            ...
            for ptm_type in self.model.ptm_types:
                wt_prob = (
                    wt_results[(wt_results["position"] == position) & (wt_results["ptm_type"] == ptm_type)][
                        "probability"
                    ].values
                    if len(wt_results) > 0
                    else [0]
                ...
                mut_prob = (
                    mut_results[(mut_results["position"] == position) & (mut_results["ptm_type"] == ptm_type)][
                        "probability"
                    ].values
                    if len(mut_results) > 0
                    else [0]
                )
...
def _build_parser():
    """构建命令行参数解析器。"""
    parser = argparse.ArgumentParser(description="批量PTM位点预测（预测蛋白质序列中的 PTM 位点）")
    ...
    fasta_parser.add_argument(
        "--model-type", default="cnn_multitask", choices=["cnn_multitask", "esm2_t12", "esm2_t6"], help="模型类型"
    )
    fasta_parser.add_argument(
        "--ptm-types",
        nargs="+",
