...
def main():
    parser = argparse.ArgumentParser(description='批量PTM位点预测')

    # 子命令
    subparsers = parser.add_subparsers(dest='command', help='预测模式')

    # FASTA批量预测
    fasta_parser = subparsers.add_parser('fasta', help='从FASTA文件预测')
    ...
    # 单序列预测
    single_parser = subparsers.add_parser('single', help='单序列预测')
    ...
    # 变异预测
    variant_parser = subparsers.add_parser('variant', help='变异PTM效应预测')
